...
export default async function handler(req: any, res: any) {
  ...
  if (req.method === "POST") {
    const b = req.body ?? {};
    if (!b.name || !b.phone || !b.date || !b.time) { res.status(400).json({ error: "name, phone, date and time required" }); return; }
    const rows = await sql`
      INSERT INTO bookings (name, phone, email, date, time, status, notes)
      VALUES (${b.name}, ${b.phone}, ${b.email ?? ""}, ${b.date}, ${b.time}, ${b.status ?? "pending"}, ${b.notes ?? ""})
      RETURNING *`;
