...
import { blogsStore, BlogPost } from "@/lib/admin-store";
...
export default function AdminBlog() {
  const [posts, setPosts] = useState<BlogPost[]>(blogsStore.get());
  const [view, setView] = useState<"list"|"edit">("list");
  const [editing, setEditing] = useState<BlogPost | null>(null);
  const [form, setForm] = useState({ title:"", slug:"", category:CATEGORIES[0], excerpt:"", content:"", published:false });

  function refresh() { setPosts(blogsStore.get()); }

  function openNew() {
    setEditing(null);
    setForm({ title:"", slug:"", category:CATEGORIES[0], excerpt:"", content:"", published:false });
    setView("edit");
  }
  ...
  function handleSave(e: React.FormEvent) {
    e.preventDefault();
    if (editing) {
      blogsStore.update(editing.id, form);
    } else {
      blogsStore.add(form);
    }
    refresh();
  ...
  function handleDelete(id: string) {
    if (confirm("Delete this post?")) { blogsStore.delete(id); refresh(); }
  }

  function togglePublish(id: string, current: boolean) {
    blogsStore.update(id, { published: !current });
    refresh();
  }
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                placeholder="Brief description shown on blog page..."
          ...
          <div className="space-y-5">
            <div className="rounded-2xl border border-white/5 bg-[#060912] p-5 space-y-4">
              ...
              <div>
                ...
                <select value={form.category} onChange={e => setForm({...form, category:e.target.value})} className="w-full bg-[#0d1220] border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-blue-500/50">
              ...
              <button type="submit" className="w-full h-10 btn-premium rounded-xl text-sm font-semibold text-white flex items-center justify-center gap-2">
          ...
          {posts.map(post => (
            <div key={post.id} className="flex items-center gap-4 px-6 py-4 hover:bg-white/[0.02] transition-colors">
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                <button onClick={() => openEdit(post)} className="p-2 rounded-lg text-gray-500 hover:text-blue-400 hover:bg-blue-500/10 transition-colors">
                  <Edit2 className="w-4 h-4" />
                </button>
                <button onClick={() => handleDelete(post.id)} className="p-2 rounded-lg text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-colors">
