...
import { Calendar, Clock, Phone, Mail, Trash2, Check, X } from "lucide-react";
import { bookingsStore, Booking } from "@/lib/admin-store";
...
export default function AdminBookings() {
  const [bookings, setBookings] = useState<Booking[]>(bookingsStore.get());
  const [filter, setFilter] = useState("all");

  function refresh() { setBookings(bookingsStore.get()); }
  ...
  function handleDelete(id: string) {
    if (confirm("Delete this booking?")) { bookingsStore.delete(id); refresh(); }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        {["all", ...STATUSES].map(s => (
          <button key={s} onClick={() => setFilter(s)} className={`px-3 py-2 text-xs font-medium rounded-xl capitalize transition-colors ${filter===s ? "bg-blue-600/30 text-blue-300 border border-blue-500/30" : "text-gray-400 border border-white/8 hover:border-white/15"}`}>
            {s}
          </button>
        ))}
      </div>
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-1 shrink-0">
                {booking.status === "pending" && (
                  <button onClick={() => updateStatus(booking.id, "confirmed")} className="p-2 rounded-lg text-gray-500 hover:text-green-400 hover:bg-green-500/10 transition-colors" title="Confirm">
                    <Check className="w-4 h-4" />
                  </button>
                )}
                {booking.status === "confirmed" && (
                  <button onClick={() => updateStatus(booking.id, "completed")} className="p-2 rounded-lg text-gray-500 hover:text-blue-400 hover:bg-blue-500/10 transition-colors" title="Mark Complete">
                    <Check className="w-4 h-4" />
                  </button>
                )}
                <button onClick={() => updateStatus(booking.id, "cancelled")} className="p-2 rounded-lg text-gray-500 hover:text-yellow-400 hover:bg-yellow-500/10 transition-colors" title="Cancel">
                  <X className="w-4 h-4" />
                </button>
                <button onClick={() => handleDelete(booking.id)} className="p-2 rounded-lg text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-colors">
