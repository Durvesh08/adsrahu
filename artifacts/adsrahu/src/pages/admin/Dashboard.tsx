...
import { Users, Calendar, FileText, Mail, TrendingUp, ArrowRight, AlertCircle, CheckCircle2, Clock } from "lucide-react";
import { leadsStore, bookingsStore, blogsStore, subscribersStore } from "@/lib/admin-store";
...
export default function AdminDashboard() {
  ...
  const stats = [
    { label: "Total Leads", value: leads.length, sub: `${newLeads} new today`, icon: Users, color: "#3b82f6", href: "/admin/leads" },
    { label: "Bookings", value: bookings.length, sub: `${pendingBookings} pending`, icon: Calendar, color: "#8b5cf6", href: "/admin/bookings" },
    { label: "Blog Posts", value: blogs.length, sub: `${publishedBlogs} published`, icon: FileText, color: "#10b981", href: "/admin/blog" },
    { label: "Subscribers", value: subs.length, sub: "Email list", icon: Mail, color: "#f59e0b", href: "/admin/subscribers" },
  ];
        ...
        <div className="divide-y divide-white/5">
          {leads.slice(0, 4).map(lead => (
            <div key={lead.id} className="flex items-center gap-4 px-6 py-3.5 hover:bg-white/[0.02] transition-colors">
              ...
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full capitalize shrink-0 ${statusColors[lead.status]}`}>
                {lead.status}
              </span>
            </div>
          ))}
